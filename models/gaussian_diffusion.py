...
class GaussianDiffusion:
    ...
    def q_sample(self, x_start, y, t, noise=None, mask=None):
        """
        ...
        with torch.no_grad():
            mask = th.where(mask <= 0.0, 0.0, 1.0)
            mask = mask.float()
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t, mask = None):
        """
    ...
    def p_mean_variance(
        ...
        model_mean, _, _ = self.q_posterior_mean_variance(
            x_start=pred_xstart, x_t=x_t, t=t, mask=mask
        )
    ...
    def p_sample_loop_progressive(
        ...
        mask = th.where(model_kwargs['mask'] <= 0.0, 0.0, 1.0)
        mask = mask.float()
    ...
    def prior_sample(self, y, noise=None, mask=None):
        """
        ...
        return y + mask.repeat(1, 3, 1, 1) * _extract_into_tensor(self.kappa * self.sqrt_etas, t, y.shape) * noise
    ...
    def training_losses(
        ...
        z_t = self.q_sample(z_start, z_y, t, noise=noise, mask=mask)
